...
public class RoutingSlipRollbackResult : IRoutingSlipRollbackResult
{
	...
	public RoutingSlipRollbackResult(RoutingSlip routingSlip, int nextCheckpointIndex,
		Action<IIntegrationEventProperties>? configureEvent)
	{
	...
	public async Task ExecuteAsync(IServiceProvider serviceProvider, IIncomingIntegrationEventProperties eventProperties,
		IIncomingIntegrationEventContext eventContext, CancellationToken cancellationToken = default)
	{
		var nextCheckpoint =
			_nextCheckpointIndex >= 0 ? _routingSlip.Checkpoints[_nextCheckpointIndex] : _routingSlip.RollbackDestination!;
