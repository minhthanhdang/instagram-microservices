...
{
	public static void AddRoutingSlip(this ITransactionalEventsContext context, Action<IRoutingSlipBuilder> configure,
	...
	public static void AddRoutingSlip(this ITransactionalEventsContext context, string? groupId,
		Action<IRoutingSlipBuilder> configure, Action<IIntegrationEventProperties>? propertiesConfigurator = null)
	{
		...
		if (firstCheckpoint == null) throw new ArgumentException("No checkpoint is added");
		...
		{
			if (propertiesConfigurator != null) propertiesConfigurator.Invoke(properties);
