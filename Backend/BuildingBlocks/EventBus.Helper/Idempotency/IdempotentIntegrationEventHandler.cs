	...
	where TQueue : IntegrationEventQueue
{
	...
	public IdempotentIntegrationEventHandler(IServiceProvider serviceProvider)
	{
		...
		_logger =
			serviceProvider.GetRequiredService<ILogger<IdempotentIntegrationEventHandler<TIntegrationEvent, TQueue>>>();
	}

	public sealed override async Task Handle(TIntegrationEvent integrationEvent,
	...
	public abstract Task HandleIdempotently(TIntegrationEvent integrationEvent,
		IIncomingIntegrationEventProperties properties, IIncomingIntegrationEventContext context);
}
