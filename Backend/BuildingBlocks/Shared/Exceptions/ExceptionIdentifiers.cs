...
public static class ExceptionIdentifiers
{
	...
	private static bool Identify(Exception ex, Dictionary<string, List<IExceptionIdentifier>> identifiersDict,
		string category, params object?[] entities)
	{
		...
		if (ex.InnerException != null) return Identify(ex.InnerException, identifiersDict, category, entities);
...
{
	private readonly Dictionary<string, List<IExceptionIdentifier>> _identifiers;
