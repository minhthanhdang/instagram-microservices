...
{
	private static ConcurrentDictionary<string, Type?> _cachedTypes = new();
	...
	{
		for (var i = 0; i < _assembliesByIndex.Count; i++)
		{
	...
	public static Type? GetType(string name, string? assemblyHint)
	{
		...
		{
			if (!TryDirectTypeLookup(assemblyHint, name, out type)) TryIndirectTypeLookup(name, out type);
