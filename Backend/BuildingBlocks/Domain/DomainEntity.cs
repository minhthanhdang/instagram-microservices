...
public class DomainEntity : Entity, IDomainEventEmitter
{
	...
	public void AddUniqueDomainEvent(IDomainEvent domainEvent)
	{
		lock (_domainEvents)
		{
			_domainEvents.RemoveAll(x => { return x.GetType() == domainEvent.GetType(); });
			_domainEvents.Add(domainEvent);
		}
	}
	...
	{
		if (rule.IsBroken()) throw BusinessRuleValidationException.Create(rule);
	}
	...
	{
		foreach (var rule in rules) CheckRule(rule);
	}
}