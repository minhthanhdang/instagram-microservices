...
public static class TransactionalEventsContextExtensions
{
	...
	private static TransactionalEvent CreateEvent(ITransactionalEventsContext context, IntegrationEventBase @event,
		Action<IIntegrationEventProperties>? propertiesConfigurator)
	{
		if (_propertiesType == null)
		{
			var eventBus = context.ServiceProvider.GetRequiredService<IEventBus>();
			if (eventBus == null) throw new ArgumentNullException("Event bus is not added");

			Interlocked.CompareExchange(ref _propertiesType, eventBus.GetIntegrationEventPropertiesType(), null);
		}

		var
			properties = (Activator.CreateInstance(_propertiesType) as IIntegrationEventProperties)!;
