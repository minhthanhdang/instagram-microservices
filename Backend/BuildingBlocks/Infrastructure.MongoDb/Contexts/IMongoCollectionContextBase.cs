...
public interface IMongoCollectionContext<TDocument> : IMongoCollectionContextBase
{
	...
	void UpdateOne(Expression<Func<TDocument, bool>> filter, UpdateDefinition<TDocument> update,
		UpdateOptions? options = null);

	void UpdateOne(FilterDefinition<TDocument> filter, UpdateDefinition<TDocument> update, UpdateOptions? options = null);

	Task<TDocument> FindOneAndLockAsync(Expression<Func<TDocument, bool>> filter,
		FindOneAndUpdateOptions<TDocument>? options = null, CancellationToken cancellationToken = default);

	Task<TDocument> FindOneAndLockAsync(FilterDefinition<TDocument> filter,
		FindOneAndUpdateOptions<TDocument>? options = null, CancellationToken cancellationToken = default);

	Task<TProjection> FindOneAndLockAsync<TProjection>(FilterDefinition<TDocument> filter,
