...
public static class MongoCollectionHelper
{
					...
					try
					{
						...
						if (collection != null)
						{
							...
							await retryPolicy.ExecuteAsync(async () =>
							{
								...
								{
									var seedTask =
										seedMethod!.Invoke(seederInstance, new[] { services, collection, collectionLogger }) as Task;
									await seedTask!;
								}
								catch (Exception ex)
								{
	...
	public static async Task SeedMongoCollection<TDocument>(this IHost app,
		Func<IMongoCollection<TDocument>, IServiceProvider, ILogger<IMongoCollection<TDocument>>, Task> seedingTask)
	{
		using var scope = app.Services.CreateScope();
		var services = scope.ServiceProvider;
