...
{
	private readonly Dictionary<string, List<TransactionalEvent>> _transactionalEventsGroup;
	...
	public Task AddToContextAsync(Dictionary<string, List<TransactionalEvent>> eventGroups,
		TimeSpan? availableDelay = null, CancellationToken cancellationToken = default)
	{
		...
		foreach (var groupId in groupIds) CommitGroup(context, groupId, eventGroups[groupId], availableDelay);
	...
	{
		if (string.IsNullOrWhiteSpace(groupId)) groupId = _defaultEventsGroupId;
	...
	public void CommitGroup(IMongoCollectionContext<TransactionalEventsGroup> context, string groupId,
		List<TransactionalEvent> transactionalEvents, TimeSpan? availableDelay = null)
	{
		if (transactionalEvents.Count == 0) return;
