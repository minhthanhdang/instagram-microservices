...
public class TransactionalEventsProcessor : ITransactionalEventsProcessor
{
	...
	private async Task ProcessEventsGroup(IMongoCollectionContext<TransactionalEventsGroup> context,
		TransactionalEventsGroup group, CancellationToken cancellationToken = default)
	{
		try
		{
			await Task.WhenAll(
				_handlers.Select(handler =>
					handler.ProcessTransactionalEventsAsync(group.TransactionalEvents, cancellationToken)));
		...
		catch (Exception ex)
		{
			...
			_logger.LogError(ex,
				"An error occurred when processing events group ({GroupId}). Processing will be retried later.", group.Id);
		}
	}

	private async Task<List<TransactionalEventsGroup>> RetrieveEventsGroups(
		IMongoCollectionContext<TransactionalEventsGroup> context, string selectionToken,
	...
	private async Task<string?> PollEventsGroupIds(IMongoCollectionContext<TransactionalEventsGroup> context,
		int fetchCount, CancellationToken cancellationToken = default)
	{
		...
		do
		{
			...
			if (findResult.Count > 0)
			{
				...
				_logger.LogTrace("Selected events group {SelectedGroupCount} out of {GroupCount}", updateResult.ModifiedCount,
					findResult.Count);

				if (updateResult.ModifiedCount > 0) return selectionToken;
