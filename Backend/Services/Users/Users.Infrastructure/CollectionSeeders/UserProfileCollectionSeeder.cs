...
{
	public override async Task SeedAsync(IServiceProvider services, IMongoCollection<UserProfile> collection,
		ILogger<IMongoCollection<UserProfile>> logger)
	{
		var indexBuilder = Builders<UserProfile>.IndexKeys;

		var indexes = new List<CreateIndexModel<UserProfile>>
		{
			new(indexBuilder.Ascending(nameof(UserProfile.Username)),
